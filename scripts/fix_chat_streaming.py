# deepseek-code/fix_chat_streaming.py
#!/usr/bin/env python3
"""Fix chat streaming display issue"""

def fix_chat_streaming():
    """Fix the chat streaming display in final_main.py"""
    file_path = "src/deepseek_code/cli/final_main.py"
    
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Find the chat streaming section and fix it
    if 'for chunk in ai_engine.chat_completion(' in content:
        # Replace the problematic streaming section
        old_streaming_section = '''            # Display AI response with streaming
            ui.console.print()
        ...
        new_streaming_section = '''            # Display AI response with streaming
            ui.console.print()
            with ui.console.status("[bold green]Thinking...[/bold green]"):
                try:
                    response_text = ""
        ...
        print("✅ Fixed chat streaming display")
        return True
    
    return False

def create_improved_chat_version():
    """Create an improved version with better streaming handling"""
    improved_chat = '''#!/usr/bin/env python3
...
'''
    ...
    with open("src/deepseek_code/cli/improved_chat.py", 'w', encoding='utf-8') as f:
        f.write(improved_chat)
    
    print("✅ Created improved_chat.py with better streaming")
    return True

def main():
    print("🔧 Fixing Chat Streaming Issues")
    print("=" * 40)
