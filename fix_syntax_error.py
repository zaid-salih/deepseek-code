# deepseek-code/fix_syntax_error.py
#!/usr/bin/env python3
"""Fix the syntax error in working_main.py"""

def fix_working_main():
    """Fix the syntax error in working_main.py"""
    file_path = "src/deepseek_code/cli/working_main.py"
    
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Find and fix the unterminated string
    # The issue is likely in the multi-line string around line 115
    # Let's replace the problematic section
    
    # Find the chat function and fix the system message
    if 'ui.console.print("' in content:
        # Replace the problematic line with proper formatting
        lines = content.split('\n')
        fixed_lines = []
        
        for i, line in enumerate(lines):
            if 'ui.console.print("' in line and not line.endswith('")'):
                # This is the problematic line - fix it
                if i + 1 < len(lines) and lines[i + 1].strip().startswith('^'):
                    # Skip the next line if it's the error marker
                    continue
                # Replace with proper print statement
                fixed_lines.append('            ui.console.print("Ready for questions! Type \'quit\' to exit.")')
            else:
    ...
    print("✅ Fixed syntax error in working_main.py")
    return True

def create_correct_working_main():
    """Create a correctly formatted working_main.py"""
...
'''
    ...
    print("✅ Created correctly formatted working_main.py")
    return True
...
def main():
    ...
    # Then create a fresh correct version
    create_correct_working_main()
